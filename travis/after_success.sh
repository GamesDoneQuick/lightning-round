...
firebase database:set -P development --token "$FIREBASE_TOKEN" --data \"$TRAVIS_COMMIT\" --confirm /deploy_commit_hash || exit 1
firebase deploy -P production --token "$FIREBASE_TOKEN" || exit 1
firebase database:set -P production --token "$FIREBASE_TOKEN" --data \"$TRAVIS_COMMIT\" --confirm /deploy_commit_hash || exit 1
